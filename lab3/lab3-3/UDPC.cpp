...
int flag;//标志是在慢启动(0)还是拥塞控制(1)还是快速恢复(2)
        ...
        if (receiveBuf[2] == 1) {//当收到ACK
            ...
            if (((base + 1) % 256 <= (u_char)receiveBuf[0] && (u_char)receiveBuf[0] <= (nextseqnum % 256))
                ...
                if (flag == 1 && segcount < cwnd) {
                    ...
                    //cout << "receive ACK " << (int)((u_char)receiveBuf[0]) << endl;
                    base = base + (int)((u_char)receiveBuf[0]) - (base + 1) % 256 + 1;
                    segcount = 0;
                    dupACKcount = 0;
                    cwnd = cwnd + 1;
            ...
            if (dupACKcount == 3) {//三次重复ACK进入快速恢复阶段
                //cout << "收到3次重复ACK，快速重传，窗口：" << cwnd << endl;
...
void senddata(string filename) {
    ...
    while (true) {
        if (nextseqnum < base + cwnd && nextseqnum < pnum) {
            setPacketHead();
            //计算数据包长度
            ...
            //cout << "send " << (int)((u_char)sendBuf[0]) << endl;
